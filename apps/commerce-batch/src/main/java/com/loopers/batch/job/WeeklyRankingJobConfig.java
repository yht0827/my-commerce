...
public class WeeklyRankingJobConfig {
	...
	private static final String RANK_TYPE = "WEEKLY";
	...
	@Bean
	public Job weeklyRankingJob(final Step weeklyRankingStep) {
	...
	public Step weeklyRankingStep(
		final ItemReader<RankedProduct> weeklyReader,
		final ItemWriter<RankedProduct> weeklyWriter
	) {
	...
	@Bean
	@StepScope
	public ItemReader<RankedProduct> weeklyReader(@Value("#{jobParameters['topN']}") final Integer topN) {
		int limit = topN != null ? topN : DEFAULT_TOP_N;
		String redisKey = rankingKeyManger.getWeeklyRankingKey();

		Set<ZSetOperations.TypedTuple<String>> tuples = redisTemplate.opsForZSet()
			.reverseRangeWithScores(redisKey, 0, limit - 1L);
		List<RankedProduct> rows = toRankedProducts(tuples);

		log.info("주간 랭킹 reader 준비 완료 - redisKey={}, rowCount={}", redisKey, rows.size());
		return new ListItemReader<>(rows);
	}

	@Bean
	@StepScope
	public ItemWriter<RankedProduct> weeklyWriter(
		@Value("#{jobParameters['rankDate']}") final String rankDateParam,
		...
		return new ItemWriter<>() {
			...
			public void write(final Chunk<? extends RankedProduct> chunk) {
				...
				LocalDate rankDate = LocalDate.parse(rankDateParam);
				Date sqlDate = Date.valueOf(rankDate);

				if (dryRun) {
					log.info("[DRY-RUN] 주간 랭킹 저장 예정 - rankDate={}, count={}, rankRange={}..{}",
