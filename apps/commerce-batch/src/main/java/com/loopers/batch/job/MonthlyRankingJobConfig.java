...
public class MonthlyRankingJobConfig {
	...
	private static final String RANK_TYPE = "MONTHLY";
	...
	@Bean
	public Job monthlyRankingJob(final Step monthlyRankingStep) {
	...
	public Step monthlyRankingStep(
		final ItemReader<RankedProduct> monthlyReader,
		final ItemWriter<RankedProduct> monthlyWriter
	) {
	...
	@Bean
	@StepScope
	public ItemReader<RankedProduct> monthlyReader(@Value("#{jobParameters['topN']}") final Integer topN) {
		int limit = topN != null ? topN : DEFAULT_TOP_N;
		String redisKey = rankingKeyManger.getMonthlyRankingKey();

		Set<ZSetOperations.TypedTuple<String>> tuples = redisTemplate.opsForZSet()
			.reverseRangeWithScores(redisKey, 0, limit - 1L);
		List<RankedProduct> rows = toRankedProducts(tuples);

		log.info("월간 랭킹 reader 준비 완료 - redisKey={}, rowCount={}", redisKey, rows.size());
	...
	public ItemWriter<RankedProduct> monthlyWriter(
		...
		return new ItemWriter<>() {
			...
			public void write(final Chunk<? extends RankedProduct> chunk) {
				...
				if (dryRun) {
					log.info("[DRY-RUN] 월간 랭킹 저장 예정 - rankDate={}, count={}, rankRange={}..{}",
