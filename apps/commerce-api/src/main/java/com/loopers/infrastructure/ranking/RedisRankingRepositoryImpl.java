...
import com.loopers.domain.ranking.RedisRankingRepository;
...
public class RedisRankingRepositoryImpl implements RedisRankingRepository {
	...
	public Page<RankingItem> fetchDaily(LocalDate date, Pageable pageable) {
		...
		return fetchByKey(key, pageable);
	}

	@Override
	public Page<RankingItem> fetchWeekly(Pageable pageable) {
		String key = rankingKeyManger.getWeeklyRankingKey();
		return fetchByKey(key, pageable);
	}

	@Override
	public Page<RankingItem> fetchMonthly(Pageable pageable) {
		String key = rankingKeyManger.getMonthlyRankingKey();
		return fetchByKey(key, pageable);
	}

	@Override
	public Long fetchDailyRank(Long productId, LocalDate date) {
		String key = rankingKeyManger.getDailyRankingKey(date);
	...
	private Page<RankingItem> fetchByKey(final String key, final Pageable pageable) {
